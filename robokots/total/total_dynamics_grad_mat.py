...
from .total_dynamics_mat import total_link_inertia_mat, total_link_wrench_to_joint_wrench_mat, total_world_link_wrench_to_world_joint_wrench_mat
...
def total_coord_to_world_link_momentum_grad_mat(r : RobotStruct, state : dict, order : int = 3, dim : int = 6) -> np.ndarray:
    return total_world_link_wrench_to_world_joint_wrench_mat(r, order, dim) @ total_coord_to_link_momentum_grad_mat(r, state, order, dim)
