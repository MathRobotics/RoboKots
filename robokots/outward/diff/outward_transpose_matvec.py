...
def _transpose_apply(mat: np.ndarray, vec: np.ndarray) -> np.ndarray:
...
def _transpose_total_coord_arrange_vec(
    ...
    vec = np.asarray(vec)
    result = np.zeros(vec.shape[:-1] + (robot.joint_dof * in_order,), dtype=vec.dtype)
...
def _transpose_total_coord_to_joint_tan_vel_grad_matvec(
    ...
    vec = np.asarray(vec)
    result = np.zeros(vec.shape[:-1] + (robot.joint_dof * order,), dtype=vec.dtype)
...
def _transpose_total_joint_tan_vel_to_link_sp_vel_grad_matvec(
    ...
    n_l = dof * (order - 1)
    vec = np.asarray(vec)
    result = np.zeros(vec.shape[:-1] + (robot.joint_num * n_j,), dtype=vec.dtype)
    for i, link in enumerate(robot.links):
        link_route = []
        joint_route = []
        robot.route_target_link(link, link_route, joint_route)
...
def _transpose_selected_coord_to_link_vel_grad_matvec(
    ...
    n_ = dim_to_dof(dim) * order
    first = next(iter(link_adjoint_blocks.values()), np.zeros(n_))
    first = np.asarray(first)
    adj_joint_tan = np.zeros(first.shape[:-1] + (robot.joint_num * n_,), dtype=first.dtype)
    for link_name, adj_link in link_adjoint_blocks.items():
        adj_link = np.asarray(adj_link)
...
def _transpose_total_link_inertia_matvec(
    ...
    for i, link in enumerate(robot.links):
        ...
        result[..., start:start+n_] = _transpose_apply(inertia, vec[..., start:start+n_])
    return result
...
def _transpose_total_factorial_matvec(num: int, vec: np.ndarray, order: int, submat_dim: int = 6) -> np.ndarray:
    ...
    for i in range(num):
        start = i * n_
        result[..., start:start+n_] = (mat @ vec[..., start:start+n_, None])[..., 0]
    return result
...
def _transpose_total_joint_wrench_to_joint_torque_matvec(
    ...
    for i, joint in enumerate(robot.joints):
        ...
        result[..., i*n_:(i+1)*n_] += (select @ vec[..., in_start:in_end, None])[..., 0]
    return result
...
def _transpose_total_partial_momentum_to_force_grad_matvec(
    ...
    vec = np.asarray(vec)
    result = np.zeros(vec.shape[:-1] + (robot.link_num * m_,), dtype=vec.dtype)
    for i, link in enumerate(robot.links):
        in_start = i * n_
        out_start = i * m_
...
def _transpose_joint_partial_momentum_to_force_grad_matvec(
    ...
    m_ = dim_to_dof(dim) * (force_order + 1)
    vec = np.asarray(vec)
    result = np.zeros(vec.shape[:-1] + (robot.joint_num * m_,), dtype=vec.dtype)
    for i, joint in enumerate(robot.joints):
        in_start = i * n_
        out_start = i * m_
...
def _transpose_total_partial_link_tan_vel_to_joint_momentum_grad_matvec(
    ...
    adj_factor = _transpose_total_factorial_matvec(robot.joint_num, vec, order-1, dof)
    n_ = dof * (order - 1)
    result = np.zeros(adj_factor.shape[:-1] + (robot.joint_num * n_,), dtype=adj_factor.dtype)
...
def outward_kinematics_jacobian_transpose_matvec(
    ...
    for st in kine_state_type_list:
        ...
        if link.name not in link_adjoint_blocks:
            link_adjoint_blocks[link.name] = np.zeros(output.shape[:-1] + (dim_dof * max_time_order,), dtype=output.dtype)
...
def outward_jacobian_transpose_matvec(
    ...
    adj_kine = np.zeros(batch_shape + (robot.link_num * dof * max_time_order,), dtype=vec.dtype)
    adj_tan_kine = np.zeros(batch_shape + (robot.link_num * dof * (max_time_order - 1),), dtype=vec.dtype)
    adj_link_mom = np.zeros(batch_shape + (robot.link_num * dof * (max_time_order - 1),), dtype=vec.dtype)
    adj_link_wmom = np.zeros_like(adj_link_mom)
    adj_joint_wmom = np.zeros(batch_shape + (robot.joint_num * dof * (max_time_order - 1),), dtype=vec.dtype)
    adj_joint_mom = np.zeros_like(adj_joint_wmom)
    adj_link_force = np.zeros(batch_shape + (robot.link_num * dof * max(force_order, 0),), dtype=vec.dtype)
    adj_joint_force = np.zeros(batch_shape + (robot.joint_num * dof * max(force_order, 0),), dtype=vec.dtype)
    adj_joint_torque = np.zeros(batch_shape + (robot.dof * max(force_order, 0),), dtype=vec.dtype)
