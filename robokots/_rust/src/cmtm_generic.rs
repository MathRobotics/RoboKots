...
impl RustCompiledRobot {
    /// Fixed/revolute high-order kinematics without 4x4 series composition
    /// and velocity recovery. R_rel^T coefficients are factorial-scaled;
    /// stored link/joint velocities are ordinary time derivatives.
    /// The rotation scratch is shared by all joints in this evaluation.
    fn kinematics_cmtm_high_order_into(&self, motion: &[f64], order: usize, ws: &mut CmtmWorkspace) {
