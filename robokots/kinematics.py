...
class Kinematics:
  ...
  def kinematics(link, joint, parent, motinos, state):
    ...
    rel_frame = Kinematics.link_rel_frame(link, joint, parent, joint_coord)
  ...
  def vel_kinematics(link, joint, parent, motinos, state):
    ...
    if parent:
      ...
      link_vel = np.zeros(6)
    rel_frame = Kinematics.link_rel_frame(link, joint, parent, joint_coord)
    rel_vel = Kinematics.link_rel_vel(link, joint, joint_veloc)
  ...
  def acc_kinematics(link, joint, parent, motinos, state):
    ...
    rel_frame = Kinematics.link_rel_frame(link,  joint, parent, joint_coord)
    rel_vel = Kinematics.link_rel_vel(link, joint, joint_veloc)
    rel_acc = Kinematics.link_rel_acc(link, joint, joint_accel)
